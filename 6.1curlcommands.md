...
'{
  ...
  "scope":"file",
  "types":["preview"],
  "toolUrl":"https://gdcc.github.io/dataverse-previewers/previewers/v1.5/ROCratePreview.html",
  "toolParameters": {
      "queryParameters":[
        {"fileid":"{fileId}"},
        {"siteUrl":"{siteUrl}"},
...
### 3D model Previewer, only x3d for now (beta)

```bash
curl -X POST -H 'Content-type: application/json' http://localhost:8080/api/admin/externalTools -d \
'{
  "displayName":"Read X3D",
  "description":"Read the X3D file.",
  "toolName":"X3D Previewer",
  "scope":"file",
  "types":["preview"],
